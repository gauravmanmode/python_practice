...
from kivy.uix.screenmanager import(
    Screen, ScreenManager, TransitionBase, FadeTransition, WipeTransition, NoTransition)
        ...
        try:
            if self.ope == "+":
                ...
                try:
                    ...
                    # denominatior cannot be 0
                    app.errors("ERROR! Denominator cannot be 0 ")
                    self.res = self.num1
            elif self.ope == "%":
                self.res = self.num1 % self.num2
            elif self.ope == "^":
...
class MainApp(MDApp):
    ...
    def change_color(self, chosen_btn):
        ...
        # Adding animations
        an = Animation(md_bg_color=[0, 0, 0, 1],
                       text_color=[1, 1, 1, 1], d=0.1)
        an += Animation(md_bg_color=[1, 1, 1, 1],
                        text_color=[0, 0, 0, 1], d=0.1)
        an.start(item)

    # implementation of logic for scientific operations
